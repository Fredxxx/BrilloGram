...
name = "90deg_tabea_32x32_gaussSheet1"
...
sf = 0.23/0.075#2.3/2.5 #2.3
#scale_factors = (sf, sf, 4*sf)
scale_factors = (sf, sf, 4*sf)
scatVol = zoom(scatVol, scale_factors, order=1)  # order=1 = linear interpolation
padded_scatVol = bf.genPaddArray3(optExc.Nx, optExc.Nx, optExc.Nx, scatVol)
#bf.plot_max_projections2(padded_scatVol, voxel_size=(optExc.dx, optExc.dx, optExc.dx), cmap='hot', title="padded_scatVol", space="real")
del scatVol
...
# start = center - optExc.Nx // 2
# end = center + optExc.Nx // 2
# t = padded_scatVol[
#     start[0]:end[0], 
#     start[1]:end[1], 
#     start[2]:end[2]
# ]
# bf.plot_max_projections2(t, voxel_size=(optExc.dx, optExc.dx, optExc.dx), cmap='hot', title="padded_scatVol", space="real")

#%%
# psfDr = bf.rotPSF(psfD, 90)
# psfEr = bf.rotPSF(psfE, 90)
# psfSysr = psfEr*psfDr
# bf.plot_max_projections2(bf.fftgpuPS(psfSysr), voxel_size=(optExc.dx, optExc.dx, optExc.dx), cmap='hot', title="ps system", space="fft")
# resS = bf.calcMain(bf.fftgpuPS(psfSysr), 'sys', theta, phi, optDet, optGen, 95, [0,0,0,0,0,0], 0, 0, 0, path)
...
# bf.saveHisto(resS, path, optDet)
# resS = bf.calcMain(bf.fftgpuPS(psfSysRot), 'sys', theta, phi, optDet, optGen, 90, [0,0,0,0,0,0], 0, 0, 0, path)
# bf.saveHisto(resS, path, optDet)
# %%save tiffs
